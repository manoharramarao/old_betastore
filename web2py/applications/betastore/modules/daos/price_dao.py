...
def get_price(product_code, group_code, price_type_code):
    ...
    row = current.db((current.db.bis_price.product_code == product_code) &
                     (current.db.bis_price.user_group_code == group_code) &
                     (current.db.bis_price.price_type_code == price_type_code)).select().first()
    if row is not None:
